...
__global__ void sgemm_coalese(int M, int N, int K, float alpha, float *A, const float *B, float beta, float *C){
  // looking at the C matrix, we want to coalesce across the x dimension
  // this means that the C matrix's threads 
  // C matrix is still a "1D array", which is how threads get grouped into warps
  const uint y = blockIdx.x * blockDim.x + threadIdx.x;
  const uint x = blockIdx.y * blockDim.y + threadIdx.y;
  //   same x value (same row in matrix C)
  // consecutive y values (consecutive columns in matrix C)
  // could also use - with 1d thread indexing
  // x dimension is indexed 
  // y dimension is indexed

  // by def threads with same threadIdx.y, and consecutive threadIdx.x, are in the same warp

  // const int x = blockIdx.x * BLOCKSIZE + (threadIdx.x / BLOCKSIZE);
  // const int y = blockIdx.y * BLOCKSIZE + (threadIdx.x % BLOCKSIZE);
  // threads only have threadIdx.x, and they will be truncated by / and % properties
  // -> coalesed across the x dim (same row)
  
  // By changing y and x, then we loop over the same row, with diff cols allowing for coalescing (see the for loop)
  // adj threads now have the same y -> i.e in the same row
  
  // check if the thread is in bounds for our matrix
  if (x < M && y < N){
    float tmp = 0.0;
    // first matrix accesses across the row
    // second matrix matrix accesses across the col
    // Before changing x and y, as x increases by 1, we go down the row
    // THREADS CONSECUTIVE TO EACHOTHER WILL ONLY BE WARPED. 
    for (int i = 0; i < K; ++i){
      tmp += A[x * K + i] * B[i * N + y];
    }
...
int main(){
  ...
  for (int i = 0; i < num_iterations; ++i){
    ...
    CHECK_CUDA_ERROR(cudaMemcpy(d_C, h_C, M * N * sizeof(float), cudaMemcpyHostToDevice));
    sgemm_coalese<<<gridDim, blockDim>>>(M,N,K, alpha, d_A, d_B, beta, d_C); // CORRECT
  }
