...
// kernel optimizations - reduce warp divergence, unroll loops for ilp, check if thread count needs increase due to arithmetic intensity saturation
__global__ void sgemm_2d_blocktiling(int M, int N, int K, float alpha,
  ...
  // total threads = (block_tile_rows * block_tile_cols) / (thread_tile_rows * thread_tile_cols) = (128 * 128) / (8 * 8) = 256 threads per block
...
int main(){
  ...
  for (int i = 0; i < num_iterations; ++i){
    ...
    CHECK_CUDA_ERROR(cudaMemcpy(d_C, h_C, M * N * sizeof(float), cudaMemcpyHostToDevice));
    sgemm_2d_blocktiling<<<gridDim, blockDim>>>(M,N,K, alpha, d_A, d_B, beta, d_C); // CORRECT
  }
